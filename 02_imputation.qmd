...
**1.  Let us multiply impute the missing data in the `nhanes` dataset**
...
pred
```
The object `pred` contains the predictor matrix from an initial run of `mice` with zero iterations, specified by `maxit = 0`. An equivalent workflow for creating a predictor matrix is with the function `make.predictorMatrix()`. 
```{r, cache=FALSE}
